...
Safety (on by default):
  1. every /mcp request needs a Bearer token (Authorization header or ?token=)
  2. file operations are confined to BRIDGE_ROOT; ../ escapes are rejected
  3. run_command has a denylist for destructive commands
...
class BearerAuthMiddleware:
    """Bearer auth middleware guarding /mcp* only.

    Accepts either:
      1. header  Authorization: Bearer <TOKEN>
      2. query   ?token=<TOKEN>   (for connector UIs with no auth field)
    """
    ...
    async def __call__(self, scope, receive, send):
        if scope["type"] == "http" and scope.get("path", "").startswith("/mcp"):
            ...
            ok = headers.get("authorization", "") == f"Bearer {self.token}"
            if not ok:
                qs = dict(parse_qsl(scope.get("query_string", b"").decode("latin-1")))
                ok = qs.get("token", "") == self.token
            if not ok:
...
_DEFAULT_ALLOWED_HOSTS = [
    "YOUR-HOST.example.com", "YOUR-HOST.example.com:*",
    "YOUR-ALT-HOST.example.com", "YOUR-ALT-HOST.example.com:*",
    "127.0.0.1", "127.0.0.1:*",
    "localhost", "localhost:*",
    "[::1]", "[::1]:*",
]
_DEFAULT_ALLOWED_ORIGINS = [
    ...
    "https://YOUR-ALT-HOST.example.com",
...
try:
    _mcp_app = server.streamable_http_app(stateless_http=True, **_ts_kwargs)
except TypeError:  # older signature: degrade one keyword at a time
    try:
        _mcp_app = server.streamable_http_app(**_ts_kwargs)
    except TypeError:
