...
def run_with_graceful_timeout(
    ...
    # Do not let child processes pop up a black console window.
    # CREATE_NO_WINDOW and CREATE_NEW_PROCESS_GROUP can coexist
    # (only DETACHED_PROCESS conflicts), so graceful shutdown still works.
    creationflags |= getattr(subprocess, "CREATE_NO_WINDOW", 0x08000000)
    kwargs["creationflags"] = creationflags
    # Belt and braces: if some path ignores creationflags, STARTUPINFO hides it.
